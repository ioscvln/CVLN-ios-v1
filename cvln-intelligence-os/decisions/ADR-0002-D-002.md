---
title: ADR-0002 — CURRENT is not TARGET
purpose: Architecture decision record for D-002.
...
# ADR-0002 — CURRENT is not TARGET

## Context

The v1.1 baseline freeze requires that every binding rule of the CVLN Intelligence OS
exists as a decision record rather than as tribal convention. D-002 is one of the
fourteen foundational decisions listed in `decisions/DECISION-REGISTRY.md`.

## Decision

Observed state and intended state are separate namespaces and never merge.

Type: `PRINCIPLE`. Scope: `whole corpus`.

## Evidence

`audit/COMPONENT-MATRIX.md`
