---
title: ADR-0007 — Proof layer is digital evidence only
purpose: Architecture decision record for D-007.
...
# ADR-0007 — Proof layer is digital evidence only

## Context

The v1.1 baseline freeze requires that every binding rule of the CVLN Intelligence OS
exists as a decision record rather than as tribal convention. D-007 is one of the
fourteen foundational decisions listed in `decisions/DECISION-REGISTRY.md`.

## Decision

The OS produces evidence packages; it does not produce legal attestation.

Type: `ARCHITECTURE`. Scope: `proof/`.

## Evidence

`META backend/server.py notarizations`
