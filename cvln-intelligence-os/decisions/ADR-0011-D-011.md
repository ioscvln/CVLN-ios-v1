---
title: ADR-0011 — Doctrine ownership is contested and frozen as a contradiction
purpose: Architecture decision record for D-011.
...
# ADR-0011 — Doctrine ownership is contested and frozen as a contradiction

## Context

The v1.1 baseline freeze requires that every binding rule of the CVLN Intelligence OS
exists as a decision record rather than as tribal convention. D-011 is one of the
fourteen foundational decisions listed in `decisions/DECISION-REGISTRY.md`.

## Decision

C-002 is not resolved by fiat; it stays an open contradiction until an ADR closes it.

Type: `GOVERNANCE`. Scope: `architecture/`.
