---
title: ADR-0010 — Zero-trust between layers
purpose: Architecture decision record for D-010.
...
# ADR-0010 — Zero-trust between layers

## Context

The v1.1 baseline freeze requires that every binding rule of the CVLN Intelligence OS
exists as a decision record rather than as tribal convention. D-010 is one of the
fourteen foundational decisions listed in `decisions/DECISION-REGISTRY.md`.

## Decision

Layer-to-layer calls authenticate; internal position grants no privilege.

Type: `SECURITY`. Scope: `security/`.

## Evidence

`META backend/server.py JWT`
