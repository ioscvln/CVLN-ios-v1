---
title: ADR-0003 — Human authority is final
purpose: Architecture decision record for D-003.
...
# ADR-0003 — Human authority is final

## Context

The v1.1 baseline freeze requires that every binding rule of the CVLN Intelligence OS
exists as a decision record rather than as tribal convention. D-003 is one of the
fourteen foundational decisions listed in `decisions/DECISION-REGISTRY.md`.

## Decision

Autonomy is bounded by gates; a human decision of record outranks any agent output.

Type: `PRINCIPLE`. Scope: `runtime, agents`.

## Evidence

`FACTORY backend/gate_routes.py`
