...
## PATCH-003-ANCHORING-AND-OPEN-QUESTIONS (v1.1-patch.3)

Corpus: **151 documents**, section `governance/` added. Traced by
`decisions/ADR-0020-D-020.md` (anchoring) and `ADR-0021-D-021.md` (open questions);
registered in `audit/freeze-manifest.yaml` under `post_freeze_patches`.

### External anchoring (OpenTimestamps, provider-neutral)
- `backend/lib/anchoring.py`: providers `ots` (IMPLEMENTED, public Bitcoin calendars, no
  credentials) and `rfc3161` (TARGET, returns `unavailable` — reserved so a qualified
  authority can be **added alongside**, never replacing OTS). Records are keyed by
  (digest, provider) so both can coexist for one package.
- States: `pending` (calendar accepted) → `confirmed` (Bitcoin attestation merged) ·
  `offline` (no calendar reachable) · `unavailable` (provider not configured). Only
  `confirmed` sets `anchored_at` in the evidence package.
- Storage: derived artefacts in `cvln-intelligence-os/audit/anchors/` (`index.json` +
  one `.ots` per digest). Env: `OTS_CALENDARS`, `OTS_TIMEOUT_SECONDS`.
- Endpoints: `GET /api/docs/anchor/providers`, `GET /api/docs/anchors`,
  `POST /api/docs/anchor/{baseline}?provider=`, `POST /api/docs/anchor/{digest}/upgrade`
  (404 unknown), `GET /api/docs/anchor/{digest}/verify` (structural only). Export now
  carries `anchor` + `anchor_proof_ots_base64`.
- **Vocabulary rule (INV-016)**: an anchor is *independent evidence of temporal existence
  and integrity* — never a qualified/eIDAS/legally opposable timestamp.
  `legal_effect` stays `"none"`. Portal panel: `frontend/src/components/AnchorPanel.tsx`
  on `/freeze`.

### Open-questions register
- `governance/OPEN-QUESTIONS.md` (43 rows) generated by `scripts/gen_open_questions.py`
  from every registry: kinds `freeze-blocker` (16), `risk-needs-decision` (4),
  `unknown-needs-evidence` (23), plus `contradiction` / `open-question`.
- `Owner` = UNASSIGNED and `Due` = TBD by default and **preserved verbatim** across
  regenerations (merge by question ID) — no tool invents accountability (D-021).
- Registry key `open-questions` on `/api/docs/registry/{key}`; portal page
  `/open-questions` (nav "Open Questions") with kind filters, blocker notice and stats.

### New invariants
INV-015 (every freeze-blocker row has Owner and Due fields) and INV-016 (no anchoring
artefact described as qualified/eIDAS). Total: **16/16 hold**.
